...
def _score(cost: float, dur: int, dist: float, mode: TransportMode) -> float:
    """
    Score for scenic preference. Lower = better for cost/time.
    """
    # Placeholder — scenic scoring can be expanded with POI density data
    return -(cost + dist * 0.5)
...
class RoutingEngine:
    ...
    def plan(self, inp: TravelInput) -> TravelOutput:
        ...
        for stop in stops:
            ...
            # Skip same-city self-loops (zero-length, zero-cost segments)
            if _same_city_seg(best_seg):
                route.append(best_seg)
                cost += best_seg.cost
                time += best_seg.duration_minutes
                dist += best_seg.distance_km
                prev_city = stop.city
                prev_lat  = stop_lat
                prev_lng  = stop_lng
                continue

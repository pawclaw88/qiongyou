...
import sys
sys.path.insert(0, '/mnt/c/Users/claudbox888/Desktop/qiongyou')
...
class TestRoutingPlan:
    ...
    def test_budget_enforcement(self):
        inp = make_input(
            budget=600.0,   # Beijing→Shanghai HSR is ¥553, budget just above it
            modes=[TransportMode.TRAIN],
            start_date=TODAY.isoformat(), end_date=NEXT_MONTH,
        )
        out = self.engine.plan(inp)
        assert out.status == Status.SUCCESS
        assert out.summary["total_cost"] <= 600.0
